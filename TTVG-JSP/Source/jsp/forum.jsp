...
<%
	...
	item2.setContent("Content Two: Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce a dui faucibus, feugiat quam vel, interdum tortor.");
...
%>
			<div class="forum-item-container">
				<div class="title">
					...
					<span><a href=""><%=item.getTitle()%></a>&nbsp;&nbsp;<small>(<%=item.getContent().length()%> bytes)&nbsp;&nbsp;<em>--<%=item.getUser()%>--</em>&nbsp;&nbsp;<%=item.getTime()%></small></span>
				</div>
				<div class="forum-text">
					<p><%=item.getContent()%> 
					</p>
					<p class="comment-count"><a href=""><%=item.getChildren().size()%> comments</a></p>
				</div>
			</div>
<%
	}
