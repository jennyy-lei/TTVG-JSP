...
CREATE TABLE IF NOT EXISTS account ( Id int unsigned not null, PersonId int unsigned not null, Role char(128) binary DEFAULT '', Email char(128) binary DEFAULT '' NOT NULL UNIQUE, Password char(128) binary DEFAULT '' NOT NULL, PRIMARY KEY Id (Id), FOREIGN KEY (PersonId) REFERENCES person (Id) ) engine=MyISAM CHARACTER SET utf8 COLLATE utf8_bin comment='Database privileges';
