...
-- Define all persons
CREATE TABLE IF NOT EXISTS person ( Id int unsigned not null, FatherId int, MotherId int, GuardianId int, GivenName char(128) binary DEFAULT '' NOT NULL, LastName char(128) binary DEFAULT '' NOT NULL, MiddleName char(128) binary DEFAULT '', Image char(128) binary DEFAULT '', Phone char(128) binary DEFAULT '', Mobile char(128) binary DEFAULT '', Address char(255) binary DEFAULT '', PRIMARY KEY Id (Id), FOREIGN KEY (FatherId) REFERENCES person (Id), FOREIGN KEY (MotherId) REFERENCES person (Id), FOREIGN KEY (GuardianId) REFERENCES person (Id) ) engine=MyISAM CHARACTER SET utf8 COLLATE utf8_bin comment='Database privileges';
